...
function printSingleRefundToken(refundObj) {
    ...
    token.innerHTML = `
        ...
        <div class="pos-divider"></div>
        <div class="meta-line" style="font-size:11px; font-weight:800; text-align:center;">
            T-${refundObj.tokenNum || 'N/A'} &nbsp;|&nbsp; ${getFormattedSystemDate()} &nbsp;|&nbsp; ${refundObj.time}
        </div>
    `;
...
// Token Printing Generator (Updated: Token positioned on the meta line next to date & time in small size)
function executeTokenPrinting(customerName) {
    ...
    for (let item in currentCart) {
        ...
        token.innerHTML = `
            ...
            <div class="pos-divider"></div>
            <div class="meta-line" style="font-size: 12px; font-weight: 900; text-align: center; color: #000000 !important; font-family: Arial, sans-serif !important;">
                TOKEN NO: ${globalTokenCounter} &nbsp;|&nbsp; DATE: ${dateStr} &nbsp;|&nbsp; TIME: ${timeStr}
            </div>
            <div style="font-size:11px; font-weight:900; margin-top:4px; text-transform:uppercase; text-align:center;">ACCOUNT MAPPING: ${customerName}</div>
        `;
        printArea.appendChild(token);
    }
    localStorage.setItem('currentDayLog', JSON.stringify(currentDayLog));
